...
def prepare_intercept(callback):
    ...
    def process_key(event_type, vk, scan_code, is_extended):
        ...
        if scan_code == 0x38 and vk == 0xA5 and is_extended:
    ...
    def low_level_keyboard_handler(nCode, wParam, lParam):
        try:
            ...
            if lParam.contents.scan_code == 541:
                return -1
...
def _send_event(code, event_type):
    ...
    if code > 0:
        ...
        num_vk = next(filter(lambda n: n[1] == code,numeric_keys), None)
        if num_vk is not None:
            vk = num_vk[2]
        ...
        if code == 0xE038:
            ...
            if event_type == 2 and ctrl_is_pressed is False:
